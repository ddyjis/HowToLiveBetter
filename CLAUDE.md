...
6. 反面清单：看起来性价比高但其实不高的东西（含通用体检套餐（Cochrane 23 万人证据说它不降死亡率，出路是第 1 节的单项筛查加国家基本公卫免费项目，编在「查出来了也别乱处理」那组之前）、转运招财物件、靠买东西调节情绪、为「在周围人里往上挪一档」加预算）
...
- 不要主动扩展目录结构，要加新节先问我
- **香港正體正文隨倉庫維護**（2026-09-22 用戶更新）：香港正體是這個 fork 的對外正文。繁化姬先生成已忽略的 `dist/zh-HK/`，檢查 diff 後再用 `node tools/promote-hk.mjs --force` 提升到根目錄的 `README.md`、`book/`、`docs/` 和本地連結的 Markdown；只提交根目錄正文。絕不手改生成的譯文。其他語言仍由譯者在自己的 fork 維護，README 只加非官方連結。
